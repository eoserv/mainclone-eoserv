	...
	for (int i = 0; i < range; ++i)
	{
		...
		UTIL_FOREACH(this->npcs, npc)
		{
			 ...
			 && npc->alive && npc->x == target_x && npc->y == target_y)
			{
				...
				{
					amount = util::rand(80 + from->hw2016_points / 2, static_cast<int>(110 + from->hw2016_points * 2)); // TODO: completion boosts?
...
void Map::SpellAttack(Character *from, NPC *victim, unsigned short spell_id)
{
	...
	if ((victim->ENF().type == ENF::Passive || victim->ENF().type == ENF::Aggressive) && victim->alive)
	{
		...
		{
			amount = util::rand(150*spell.cast_time + spell.mindam*2 + from->hw2016_points / 2, 210*spell.cast_time + spell.maxdam*2 + from->hw2016_points * 2); // TODO: Apply kill / completion boosts
